...
const defaultFormatter = result => `${result.msg} -> ${result.passed ? 'ok' : 'failed'}`;
...
const AssertionResult = ({ msg, passed, formatter }) => {
    const formatterFn = () => formatter({ msg, passed });

    return {
        msg, passed, formatterFn,
    }
};
